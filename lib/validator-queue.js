...
class ValidatorQueue {
  ...
  _validate(value, extraData) {
    const resultSucc = new Result({value});
    // value is empty and not required will return success
    if (!this._require && !RequireValidator.isNotEmpty(value)) return resultSucc;
